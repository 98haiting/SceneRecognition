...
class TestDataset(Dataset):
    def __init__(self, data, resolution):
        ...
        """
        data = data[data['image_address'] != 'Image Credits']
        imgs_in_file = os.listdir('./data/ODOR3-testset')
        imgs_in_doc = data['filepath']
        for img in imgs_in_doc:
            if img not in imgs_in_file:
                data = data[data['filepath'] != img]
        self.samples = data.reset_index(drop=True)

        self.root = './data/ODOR3-testset/'
        self.resolution = resolution
        """
        self.samples = data
    ...
    def __getitem__(self, index):
        # content = self.samples.iloc[index]
        # label = torch.tensor(int(content['labels']))
        # url = content['image_address']
        #
        ...
        """
        content = self.samples.iloc[index]
        filepath = self.root + content['filepath']
        img = imread(filepath)
        if len(img.shape) == 2:
            img = gray2rgb(img)
        image = self._transform(img)
