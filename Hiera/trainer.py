...
class Trainer:
    def __init__(self, model, train_data, val_data, criterion, optimizer, scheduler, early_stop=5, args=None, device=None):
        self._model = model
        ...
        self.f1_path = f'./checkpoints/{args.model}_f1.pkl'

    def _mixup_data(self, x, y, alpha=1.0, use_cuda=True):
    ...
    def _mixup_criterion(self, pred, y_a, y_b, lam):
        return lam * self._crit(pred, y_a) + (1 - lam) * self._crit(pred, y_b)
    ...
    def _training_epochs(self):
        ...
        f1_train = AverageMeter('Train F1', ':6.2f', Summary.AVERAGE)

        train_progress = ProgressMeter(
            len(self._train_data),
            [batch_time, train_loss, top1_train, top5_train, f1_train],
            prefix="Training: ")
        ...
        for batch_id, (image, label) in enumerate(self._train_data):
            ...
            cutmix_or_mixup = np.random.choice([cutmix, mixup])
        ...
        return top1_train.avg, top5_train.avg, f1_train.avg, train_loss.avg
    ...
    def _validation_epochs(self):
        ...
        f1_val = AverageMeter('Validation F1', ':6.2f', Summary.AVERAGE)

        val_progress = ProgressMeter(
            len(self._train_data),
            [batch_time, val_loss, top1_val, top5_val, f1_val],
            prefix="Validation: ")
        ...
        return top1_val.avg, top5_val.avg, f1_val.avg, val_loss.avg
    ...
    def fit(self, epochs=25):
        ...
        for epoch in range(epochs):
            ...
            train_loss.append(training[3])

            validation = self._validation_epochs()
            val_acc1.append(validation[0])
            val_acc5.append(validation[1])
            val_loss.append(validation[3])

            if validation[3] < self.best_loss:
                self.best_loss = validation[3]
