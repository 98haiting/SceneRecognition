...
def parse_args():
    ...
    parser.add_argument('--lr', type=float, default=2e-4, help='learning rate (default: 0.003)')
...
def main_worker(gpu, args):
    ...
    # the model is pretrained on mae only, no fine-tuning
    model = hiera.hiera_base_224(drop_path_rate=0.1, pretrained=args.pretrained, checkpoint='mae_in1k')
    resolution = 224
    if args.checkpoints:
    ...
    if not args.prediction:
        ...
        trainer = Trainer(model=model, train_data=train_dataloader, val_data=val_dataloader, criterion=criterion,
                          optimizer=optimizer, scheduler=scheduler, early_stop=5, args=args, device=device)
        print("start training")
        tok1, tok5, f1, loss = trainer.fit(epochs=args.epochs)
    ...
    if args.prediction:
        if args.local_rank == 0:
            ...
            print("start testing")
            top1_test, top5_test, f1_test = tester.predictor()
